...
plot1 <- ggplot(bakers, aes(x = technical_bottom, 
  ...
  theme_minimal() +
  geom_text(x=1, y=110, aes(label = R2), data = bakers_lm_top)
#annotate(geom="text", x=1, y=110, aes(label = paste(bakers_lm_top[1, 4]))) +
...
bakers1 <- bakers |>
  ...
  mutate(score_type = case_when(
    score_type == "technical_highest" ~ "Technical Highest",
    score_type == "technical_lowest" ~ "Technical Lowest",
    score_type == "technical_median" ~ "Technical Median"
  )) |>
  rename(amount = value) |>
  filter(age_generation != "Silent Generation") |>
  filter(age_generation != "Gen Z")
...
ggplot(data = bakers1, aes(
  ...
  labs(
    y = NULL, x = "Respective Technical Challenge Ranking",
    title = "Technical Challenge Ranking Extreme by Age Generation", 
    caption = "Gen Z and Silent Generation not included 
    \n due to small sample size (n= 4, n=1 respectively)"
  )
```
