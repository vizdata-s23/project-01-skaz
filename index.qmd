...
bakers1 <- bakers |>
  pivot_longer(cols = c(technical_highest, technical_lowest, technical_median), names_to = "score_type") |>
  mutate(score_type = case_when(score_type == "technical_highest" ~ "Technical Highest",
...
```{r}
ggplot(data = bakers, aes(x = technical_highest, fill = age_generation)) +
  geom_bar() +
  geom_bar(aes(x = technical_lowest)) +
