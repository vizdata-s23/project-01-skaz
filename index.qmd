...
(1-2 paragraphs): 
The main component of The Great British Baking Show is the technical challenges. We are interested in exploring if placing in a technical challenges affects the number of episodes a contestant appears in. We will also explore how age plays a role in this correlation. We will use the bakers data set and the variables `technical_top3`, `technical_bottom`, `percent_episodes_appeared`, `technical_lowest`, `technical_highest`, and `technical_median`. Additionally, we mutated to create a variable named `age_generation` that we will use to facet by generation. 
...
plot1 <- ggplot(bakers, aes(x = technical_bottom, 
  ...
  labs(
    ...
    x = "Number of Times Appearing in Technical Top or Bottom 3",
    title =
...
ggplot()+
  ...
  geom_point(data = ratings_ppl, aes( x = factor(series), y = viewers_7day_mean, fill = presenters),stat="identity", size=7)+
  geom_segment(data = ratings_ppl,aes(x=series, xend = series, y = 0, yend = viewers_7day_mean, color = judges), linewidth = 6)+
  theme_minimal()+
  labs(
