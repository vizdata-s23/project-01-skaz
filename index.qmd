...
bakers <- bakers |>
  ...
  mutate(age_generation = case_when(birth_year >= 1928 & birth_year <= 1945 ~ "silent",
                                birth_year >= 1946 & birth_year <= 1964 ~ "boomer",
...
```{r first-plot}
ggplot(bakers, aes(x = technical_bottom, y = percent_episodes_appeared)) +
  geom_point(shape = 1, color = "red") +
  geom_smooth(aes(x = technical_bottom), se= FALSE, linetype = "dashed", color = "red") +
  geom_point(aes(x = technical_top3), shape = 16, color = "darkgreen") +
  geom_smooth(aes(x = technical_top3), se = FALSE, color = "darkgreen") +
  facet_wrap(~age_generation)
```
