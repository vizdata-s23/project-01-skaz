...
```{r first-plot}
ggplot(bakers, aes(x = technical_bottom, y = percent_episodes_appeared, color = age_generation)) +
  geom_point(shape = 1) +
  geom_smooth(aes(x = technical_bottom), se= FALSE) +
  geom_point(aes(x = technical_top3), shape = 16) +
  geom_smooth(aes(x = technical_top3), se = FALSE)
```
