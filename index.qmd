...
## Question 2: Effects on Viewership
...
ggplot()+
  ...
  geom_point(data = ratings, aes(x = factor(series), y = viewers_7day_mean, color = presenters), size = 3)+
  theme_minimal()+
  labs(
...
--> One of the most objective measures of a TV show or Movie's success is the number of viewers. As such, we decided to investigate how changes in format and platform impact the viewership as well as how viewership changes throughout seasons. In order to properly answer these questions we drew on data from Wikipedia about the different channels, hosts, and judges, adding them to the ratings dataset as `presenters`, `judges`, and `channel` respectively. 
